...
import { 
  Search, MapPin, HeartPulse, Stethoscope, 
  History, ShieldCheck, Menu, X, Loader2, Star, 
  ChevronDown, Phone, Mail, Globe, User, LogOut
} from 'lucide-react'
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans text-gray-900">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2 font-bold text-2xl text-purple-900">
            ...
            <span className="text-lg md:text-2xl">DocSlot</span>
          </Link>
            ...
            {user ? (
              ...
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-2 hover:bg-gray-50 px-3 py-2 rounded-lg transition-all"
                >
                  <div className="w-8 h-8 bg-purple-100 text-purple-700 rounded-full flex items-center justify-center font-bold text-sm uppercase">
                    {profile?.full_name?.[0] || 'U'}
                  </div>
                  <span className="text-sm font-medium">{profile?.full_name || 'User'}</span>
                  <ChevronDown size={16} className="text-gray-400"/>
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border rounded-xl shadow-xl py-1 animate-in fade-in zoom-in-95">
                    <Link href="/profile" className="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 font-bold border-b border-gray-50">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-100 shadow-xl z-50 animate-in slide-in-from-top-5">
              ...
              {user ? (
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
                    <div className="w-10 h-10 bg-purple-100 text-purple-700 rounded-full flex items-center justify-center font-bold text-lg uppercase">
                      {profile?.full_name?.[0] || 'U'}
                    </div>
                    <div>
                  ...
                  <Link 
                    ...
                    className="flex items-center gap-3 w-full p-3 bg-gray-50 rounded-xl font-bold text-gray-700 hover:bg-green-50 hover:text-green-700 transition-colors"
                  >
                  ...
                  <button 
                    onClick={handleLogout}
                    className="flex items-center gap-3 w-full p-3 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-100 transition-colors"
                  >
                ...
                <div className="space-y-3">
                  <Link 
                    href="/join" 
                    onClick={() => setIsMenuOpen(false)}
                    className="block text-gray-600 font-medium p-2"
                  >
                    Partner with Us
                  </Link>
                  ...
                  <Link 
                    href="/auth" 
                    onClick={() => setIsMenuOpen(false)}
                    className="flex items-center justify-center w-full p-3 border border-gray-200 rounded-xl font-bold text-gray-700 hover:bg-gray-50"
                  >
                    Log In
                  </Link>
                  <Link 
                    href="/auth" 
                    onClick={() => setIsMenuOpen(false)}
                    className="flex items-center justify-center w-full p-3 bg-purple-900 text-white rounded-xl font-bold shadow-lg shadow-purple-200"
                  >
                    Sign Up
                  </Link>
                </div>
      ...
      {/* --- 2. HERO SECTION --- */}
      <section className="bg-green-50 pt-10 pb-16 px-4 md:pt-16 md:pb-24">
        <div className="max-w-4xl mx-auto text-center space-y-6">
          <h1 className="text-4xl md:text-6xl font-extrabold text-gray-900 leading-tight">
            Book your appointment <br/>
            <span className="text-green-500">Fast & Easy.</span>
          ...
          {/* Search Bar */}
          <div className="mt-8 bg-white p-2 rounded-2xl shadow-xl shadow-green-100/50 border border-gray-100 flex flex-col md:flex-row items-center gap-2 max-w-3xl mx-auto">
            <div className="flex-1 flex items-center px-4 h-14 w-full md:border-r border-gray-100 border-b md:border-b-0">
              <Search className="text-gray-400 h-5 w-5 mr-3" />
              <input 
                type="text" 
                placeholder="Doctor, Specialty..." 
            ...
            <div className="flex-1 flex items-center px-4 h-14 w-full border-b md:border-b-0">
              <MapPin className="text-gray-400 h-5 w-5 mr-3" />
              <input 
                type="text" 
                placeholder="City (e.g. Delhi)" 
      ...
      {/* --- 3. DOCTOR LIST --- */}
      <section className="max-w-7xl mx-auto px-4 py-12 md:py-16 w-full">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold text-gray-900">Top Rated Doctors</h2>
          {!loading && <span className="text-sm text-gray-500 font-medium">{doctors.length} results</span>}
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {doctors.map((doc) => (
              <div key={doc.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col group">
                <div className="flex justify-between items-start mb-4">
                  <div className="h-16 w-16 rounded-full bg-purple-50 overflow-hidden flex items-center justify-center text-purple-600 font-bold text-2xl">
                    {doc.image_url ? <img src={doc.image_url} className="h-full w-full object-cover"/> : doc.name[0]}
                  </div>
      ...
      <footer className="bg-gray-900 text-white py-12 mt-auto border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 font-bold text-2xl">
              <HeartPulse className="text-green-500" /> DocSlot
            </div>
            <p className="text-gray-400 text-sm leading-relaxed max-w-xs">
              Connecting patients with the right doctors instantly.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-gray-400">
              <li className="flex items-center gap-3">
                <Mail className="h-5 w-5 text-purple-400"/>
                <span>support@docslot.com</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="h-5 w-5 text-purple-400"/>
                <span>+91 9569528266</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="text-center pt-8 mt-8 border-t border-gray-800 text-gray-500 text-xs">
          © {new Date().getFullYear()} DocSlotBooking Inc.
        </div>
      </footer>
...
function FeatureCard({ icon, title, desc }: any) {
  return (
    <div className="bg-gray-50 p-6 md:p-8 rounded-2xl hover:bg-green-50 transition-colors border border-transparent hover:border-green-100">
      <div className="bg-white w-14 h-14 rounded-xl flex items-center justify-center shadow-sm mb-6 text-xl">
        {icon}
      </div>
      <h3 className="font-bold text-lg text-gray-900 mb-2">{title}</h3>
